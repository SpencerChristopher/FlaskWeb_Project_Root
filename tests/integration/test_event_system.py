import pytest
from src.events import user_role_changed, article_published, article_created, article_updated, article_deleted
    ...
    def test_article_published_emits_signal(self, app, signal_tracker):
        ...
        with app.app_context():
            ...
            draft_art = article_service.create_article(
                article_dto=ArticleCreateUpdate(title="Draft", content="C", summary="S", is_published=False),
                user=user_identity
            )
            ...
            with signal_tracker(article_published) as tracker:
                article_service.update_article(
                    ...
                    article_dto=ArticleCreateUpdate(title="Pub", content="C", summary="S", is_published=True),
                    user=user_identity
                )
                assert tracker.called
            author.delete()
