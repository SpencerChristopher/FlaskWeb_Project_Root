...
def test_login_sets_secure_cookies_and_omits_body_token(
    client: FlaskClient, setup_users
):
    res = client.post(
        "/api/auth/login",
    ...
    assert any(
        "access_token_cookie" in h and "HttpOnly" in h for h in set_cookie_headers
    )
    assert any(
        "refresh_token_cookie" in h and "HttpOnly" in h for h in set_cookie_headers
    )
