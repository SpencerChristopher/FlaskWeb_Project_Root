...
def test_jwt_cookie_secure_config(app: Flask):
    ...
    """
    assert app.config["JWT_COOKIE_SECURE"], "JWT_COOKIE_SECURE should be True for HTTPS environment."
...
def test_secure_flag_on_login_cookies(client, setup_users, app: Flask):
    ...
    for cookie_header in set_cookie_headers:
        if 'access_token_cookie' in cookie_header:
            ...
            assert 'Secure' in cookie_header, f"access_token_cookie missing 'Secure' flag: {cookie_header}"
            assert 'HttpOnly' in cookie_header, f"access_token_cookie missing 'HttpOnly' flag: {cookie_header}"
            assert 'SameSite=Lax' in cookie_header or 'SameSite=Strict' in cookie_header or 'SameSite=None' in cookie_header, f"access_token_cookie missing SameSite attribute: {cookie_header}"
            break
    ...
    for cookie_header in set_cookie_headers:
        if 'refresh_token_cookie' in cookie_header:
            ...
            assert 'Secure' in cookie_header, f"refresh_token_cookie missing 'Secure' flag: {cookie_header}"
            assert 'HttpOnly' in cookie_header, f"refresh_token_cookie missing 'HttpOnly' flag: {cookie_header}"
            assert 'SameSite=Lax' in cookie_header or 'SameSite=Strict' in cookie_header or 'SameSite=None' in cookie_header, f"refresh_token_cookie missing SameSite attribute: {cookie_header}"
            break
    ...
    if app.config.get("JWT_COOKIE_CSRF_PROTECT", False): # Check actual config, not manager's internal
        for cookie_header in set_cookie_headers:
            if 'csrf_access_token' in cookie_header:
                csrf_cookie_found = True
                assert 'Secure' in cookie_header, f"csrf_access_token missing 'Secure' flag: {cookie_header}"
                # CSRF token is designed to be read by JavaScript, so HttpOnly should be false
                assert 'HttpOnly' not in cookie_header, f"csrf_access_token should NOT have 'HttpOnly' flag: {cookie_header}"
                assert 'SameSite=Lax' in cookie_header or 'SameSite=Strict' in cookie_header or 'SameSite=None' in cookie_header, f"csrf_access_token missing SameSite attribute: {cookie_header}"
                break
