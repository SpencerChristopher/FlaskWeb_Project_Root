...
def test_author_only_sees_own_articles(
    client, content_admin_user, content_admin_headers, admin_user
):
...
def test_author_can_delete_own_article(
    client, content_admin_user, content_admin_headers
):
    with client.application.app_context():
        art = Article(
            title="My Art", slug="my-art", content="c", author=content_admin_user
        ).save()
