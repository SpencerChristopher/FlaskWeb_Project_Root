...
def test_user_cannot_delete_others_Article(client, setup_users, login_user_fixture):
    ...
    with client.application.app_context():
        art = Article(title="Victim Article", slug="victim-Article", content="Secret data", author=admin_user)
        art.save()
    ...
    response = client.delete(f"/api/content/articles/{article_id}", headers=attacker_headers)
