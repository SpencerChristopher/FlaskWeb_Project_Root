...
def test_register_weak_password_rejected_and_not_logged(client, admin_headers, caplog):
    ...
    ), f"Weak password should have been rejected with 400, got {response.status_code}"
...
def test_pydantic_error_details_no_echo(client, admin_headers):
    ...
    assert (
        "not-an-email" not in details_str
    ), f"Input value echoed in error details: {details_str}"
