...
class TestAuditIntegrity:
    ...
    def test_unauthorized_access_is_logged_as_security_alert(self, client, user_headers, admin_article, caplog):
        """
        ...
        with caplog.at_level(logging.WARNING):
            resp = client.delete(f"/api/content/articles/{article_id}", headers=user_headers)
    ...
    def test_article_creation_does_not_redundantly_hydrate_user(self, client, admin_headers, app):
        """
        ...
        with patch("src.repositories.mongo_user_repository.MongoUserRepository.get_by_id", side_effect=real_get_by_id) as mock_get_user:
            ...
            resp = client.post("/api/content/articles", json=payload, headers=admin_headers)
            ...
            assert mock_get_user.call_count == 1, f"Redundant User hydration detected! get_by_id called {mock_get_user.call_count} times. Expected 1 (Auth check only)."
